...
public class LevelLogic : MonoBehaviour
{
    ...
    public void EatDot(bool powerDot = false)
    {
        ...
        if (score >= targetScore) { // TRANZICIJA TUKA
            EndLevel();
            OnTargetScoreReached();
        }

        ++dotsEaten;
        if (dotsEaten == maxDots)  // if pacman ate all of the dots. NEMA DA SE DESI OVA deka score-ot ke bide reachnat pred da se izedat site dots
        {
            ++level;
    ...
    void EndLevel()
    {
        ...
        //Invoke(nameof(StartLevel), levelEndStopGhostsDuration + endLevelAnimationDuration); TRGNAV
    }
    ...
    void Update()
    {
        // start the first level once the map has finished loading
        if (startFirstLevel && mapManager.IsLoaded)
        {
    ...
    private IEnumerator ShowSuccessAndTransition()
    {
        ...
        if (t == 86.35f)
        {
            gameManager.UnloadPacManAndLoadCafeteria();
            //gameManager.UnloadFlappyBirdAndLoadClassroom("Classroom1");
        }
        else 
        {
            gameManager.UnloadPacManAndLoadClassroom("Classroom2");
            //gameManager.UnloadFlappyBirdAndLoadClassroom("Classroom2");
